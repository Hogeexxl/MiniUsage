...
fn t_dist_012_release_jobs_build_only_supported_assets() {
    ...
    for required in [
        ...
        "cargo packager --release --config \"$config_path\"",
...
fn t_dist_013_windows_release_has_static_runtime_and_install_smoke() {
    ...
    for required in [
        ...
        "PE32+",
        "$binary = Join-Path $env:GITHUB_WORKSPACE 'target/release/usagi.exe'",
        "$usagiBinary = Join-Path $releaseDir 'usagi.exe'",
        "Cargo Usagi release binary was not found",
        "productName = 'Usagi'",
...
fn t_dist_014_macos_release_has_arm64_clean_runtime_smoke() {
    ...
    for required in [
        ...
        "file target/release/usagi",
        "cp \"$release_dir/usagi\" \"$release_dir/mini-usage\"",
        "\"productName\": \"MiniUsage\"",
        "\"binaries\": [{\"path\": \"mini-usage\", \"main\": True}]",
        "cargo packager --release --config \"$config_path\"",
