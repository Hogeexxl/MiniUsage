...
# After that v5 fixture reaches latest, the next migrate call is also an
# idempotence check and must start at the current schema version.
value = value.replace("assert_eq!(migrate(&mut upgraded, 8).unwrap(), 8);", "assert_eq!(migrate(&mut upgraded, 9).unwrap(), 9);")
