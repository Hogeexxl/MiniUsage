...
export function CostValue({ value, status, ticker = false, className }: { value: number | null; status: EstimatedCostStatus; ticker?: boolean; className?: string }) {
  ...
  const valueNode = value === null ? <span className={className}>—</span> : ticker ? <NumberTicker value={Math.round(value * 100)} blur format={(next) => formatCompactCost(next / 100)} className={className} title={`$${value.toFixed(2)}`} /> : <span className={className} title={`$${value.toFixed(2)}`}>{text}</span>;
