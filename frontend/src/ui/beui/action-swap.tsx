...
export function ActionSwapText({ value, children, animation = "blur", className }: ActionSwapTextProps) {
  ...
  return (
    <span
      className={cn("relative -my-[0.08em] inline-block max-w-full whitespace-nowrap py-[0.08em] align-bottom", className)}
      ...
      <span aria-hidden className="invisible inline-block whitespace-nowrap">
        {cascade
          ? label.split("").map((char, index) => (
              <span key={`${char}-${index}`} className="inline-block whitespace-pre">{char}</span>
            ))
          : children}
      </span>
      {cascade ? (
...
export function ActionSwapButton({
  ...
  return (
    <motion.button
      ...
      className={cn(
        "inline-flex items-center justify-center overflow-hidden font-medium transition-colors disabled:pointer-events-none disabled:opacity-50",
    ...
    >
      ...
      {!iconOnly ? (
        <ActionSwapText value={activeItem.id} animation={animation}>{activeItem.label}</ActionSwapText>
      ) : null}
    </motion.button>
  );
}
