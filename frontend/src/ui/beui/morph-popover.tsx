...
export function MorphPopoverContent({
  ...
  const layout = usePopoverPortalPosition(context.triggerRef, context.contentRef, portalReady && context.open);
  ...
  const wrap = reduce ? undefined : {
    hidden: { opacity: 0, scale: 0.96, transition: SPRING_PANEL },
    show: { opacity: 1, scale: 1, transition: SPRING_PANEL },
  };
  const clip = reduce ? undefined : {
    hidden: { clipPath: clipHidden(side, align, radius), transition: MORPH_CLIP_TRANSITION },
    show: { clipPath: clipShown(radius), transition: MORPH_CLIP_TRANSITION },
  };
    ...
    <AnimatePresence>
      {context.open ? (
        <motion.div
          ...
          style={{ left, top, visibility: layout ? "visible" : "hidden", transformOrigin: originFor(side, align) }}
