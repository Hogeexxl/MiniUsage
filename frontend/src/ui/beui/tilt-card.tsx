...
export function TiltCard({ children, max = 12, glare = true, className }: TiltCardProps) {
  ...
  const glareBackground = useMotionTemplate`radial-gradient(circle at ${gx}% ${gy}%, var(--foreground), transparent 50%)`;
  ...
  return (
    ...
    >
      {children}
      {glare && enabled ? (
        <motion.div aria-hidden style={{ background: glareBackground }} className="pointer-events-none absolute inset-0 opacity-15" />
      ) : null}
    </motion.div>
  );
}
